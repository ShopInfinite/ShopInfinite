...
function MenCa({ onAddToCart }) {
  ...
  const addToCart = (product) => {
    ...
    async function sendProductToServer() {
      try {
        const response = await axios.post('http://localhost:3001/Data', product);
        setProducts(response.data);
        console.log(response.data);
      } catch (error) {
        alert('Failed to save data to the server:', error);
          ...
          {products.map((product, index) => (
            <div key={index} className="p-9">
                ...
                <article className="rounded-xl bg-white p-3 shadow-lg hover:shadow-2xl hover:transform hover:scale-105 duration-300">
                  <a href="#">
                    <div className="relative flex items-end overflow-hidden rounded-xl h-[500px]">
                    <Link to={`/Details/${product.product_id}`} >
                      <img src={product.imageUrl[0]} alt="Product Photo" /></Link>
...
//         console.log(response.data);
//       } catch (error) {
//         alert('Failed to fetch data from the server:', error);
//       }
//     }
//     getProductsFromServer();
//   }, []);
...
//     axios.post('http://localhost:3001/favorite', favorites)
//       .then((response) => {
//         console.log('Favorites updated on the server:', response.data);
//       })
//       .catch((error) => {
